...
- but don’t know what mechanics would be around it.
```
#3 (v3)
```
...
dragons 🙂 “master breeder” and “battle master”
```
#8
```
...
based on these different pieces of land and what’s in that piece of land. 
...
Also sadly don’t have that much time (not flying that often) if anyone wants to contribute to the contract dev - let me know.
But first need help with a front end for the current functionality
```
#19
```
What if your “Kingdom” was a vault that you held your separate Dragon NFTs in and for adventures/battles, you have to send your dragons out to 
attack other kingdoms aka other players vaults. Rewards based on “Damage” done to other players “kingdom”
```
#20
```
response to 19 
...
referral machanics 
main issue Bots.
verification that it’s another person not same person using bots
possible solutions
```
...
-if its possible can integrate score system from stats.gallery
     can set a specific score limits for rewards😉
...
re: lore. super important. If someone wants to take time to write a more deep lore for the DragonNEAR 
      people need to adopt to become dragonears or get extinct… there is no way back
      dragonear -> is when person merges with dragon’s mind and becomes one for a period of time.
...
-adding some single player way to play with dragons - go to training grounds or farm a 
      dangeon is important as well, to make sure PvP doesn’t get stalled because not enough active players
...
most interesting question is – how to earn money which doesn't feel like grind, but is actually fun
